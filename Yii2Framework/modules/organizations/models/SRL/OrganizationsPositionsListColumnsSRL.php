...
use app\modules\organizations\models\DAL\OrganizationsPositionsListColumns;
class OrganizationsPositionsListColumnsSRL {
    public static function getModels($where = []) {
        $query = OrganizationsPositionsListColumns::find();
        if ($where) {
            $query->where($where);
        }
