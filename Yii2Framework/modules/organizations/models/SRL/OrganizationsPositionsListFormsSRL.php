...
use app\modules\organizations\models\DAL\OrganizationsPositionsListForms;
class OrganizationsPositionsListFormsSRL {
    public static function getModels($where = []) {
        $query = OrganizationsPositionsListForms::find();
        if ($where) {
            $query->where($where);
        }
