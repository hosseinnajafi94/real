...
use app\modules\organizations\models\DAL\OrganizationsUnitsListInsuranceAcc;
class OrganizationsUnitsListInsuranceAccSRL {
    public static function getModels($where = []) {
        $query = OrganizationsUnitsListInsuranceAcc::find();
        if ($where) {
            $query->where($where);
        }
